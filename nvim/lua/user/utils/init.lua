...
M.root_patterns = { ".git", "lua", "pyproject.toml", "go.mod" }

function M.is_vscode()
	return vim.g.vscode == true or vim.g.vscode == 1 or vim.env.VSCODE ~= nil
end

---@param plugin string
---@param default boolean
function M.vscode_plugin_enabled(plugin, default)
	if not M.is_vscode() then
		return true
	end
	local key = "vscode_enable_" .. plugin
...
function M.safe_require(module)
	--- attempt to load a module using `pcall` if it fails, log an error and return nil
	local status, lib = pcall(require, module)
	if not status then
		vim.notify("Failed to load " .. module .. "\nError: " .. lib, vim.log.levels.ERROR)
		return nil
	end
	return lib
end
